...
class T5Large(nn.Module):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--model_name",
                            type=str, default='google/pegasus-xsum',
                            help="T5 large model")
        ...
        for _ in trange(int(self.args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if self.args.add_functurn_loss or self.args.add_module_loss:
                    sent_repr_mat = []
                    turn_nums = [(item == self.generator.model.config.bos_token_id).sum().cpu().item() for item in
                                 source_ids]
                    max_turn_num = max(turn_nums)
                    for i in range(batch_size):
                        sent_repr = encoder_outputs[i][
                            source_ids[i] == self.generator.model.config.bos_token_id]  # [num_of_turns, hd_dim]
                        sent_repr = torch.cat(
    ...
    def predict(self,
        ...
        for bi, batch in enumerate(tqdm(eval_dataloader, desc="Generating")):
            ...
            IDs, example_indices, source_ids, source_mask = item_dict["ID"], item_dict["example_indices"], item_dict[
                "source_ids"], item_dict["source_mask"]
    ...
    def evaluate(self,
        ...
        for metric, results in sorted(scores.items(), key=lambda x: x[0]):
            print('{}:\t{}: {:5.4f}\t{}: {:5.4f}\t{}: {:5.4f}'.format(metric, 'P', 100.0 * results['p'], 'R',
                                                                      100.0 * results['r'], 'F1', 100.0 * results['f']))
        print("\n\n")
        ...
        if dump_pred:
            with open(os.path.join(self.args.output_dir, "{}.pred.summary{}.json".format(source, self.args.add_name)),
                      "w") as fout:
                json.dump(pred, fout, indent=4)

            with open(os.path.join(self.args.output_dir, "{}.predkp.summary{}.json".format(source, self.args.add_name)),
                      "w") as fout:
...
if __name__ == '__main__':
    ...
    if model.args.wandb:
        ...
        wandb.init(project="DialSum", name=name, group="hft5l", job_type="train")
    ...
    if model.args.do_train:
        ...
        model.train()
        # model.generator = T5ForConditionalGeneration.from_pretrained(model.args.model_name, state_dict=torch.load(os.path.join(model.args.output_dir, "pytorch.bin")))
